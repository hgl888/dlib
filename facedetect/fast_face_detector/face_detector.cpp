    ...
    : m_confidence(_conf)
{
    ...
    init_detector = true;
    for(unsigned int i = 0; i < m_modelDsPad.size(); ++i)
    {
...
void FFDDetector::chnsPyramid(const ffd::structs::Pyramid &_pyr)
{
    ...
    for(unsigned int i = 0; i < isA.size(); ++i)
    {
        iA = isA.at(i) - 1;
        iR = isN.at(iA) - 1;
...
void FFDDetector::getChild(float *chns1, unsigned int *cids, int *fids, float *thrs, int offset, int &k0, int &k)
{
    //std::cout << k << std::endl;
...
void FFDDetector::init()
{
    ...
    for(unsigned int i = 1; i < nScales; i += m_cls.opts().pPyramid.nApprox + 1)
    {
        isR.push_back(i);
    }
    ...
    std::vector<int> v_j;
    v_j.push_back(0);
    for(unsigned int i = 0; i < isR.size() - 1; ++i)
    {
    ...
    #pragma omp parallel for num_threads( omp_get_num_procs() * omp_get_num_threads() )
    for(unsigned int i = 0; i < isR.size(); ++i)
    {
        for(unsigned int j = v_j.at(i); j < v_j.at(i + 1); ++j)
        {
            isN.at(j) = isR.at(i);
        }
    }
